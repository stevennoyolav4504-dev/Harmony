...
[![Version](https://img.shields.io/badge/version-2.0.1-9850FF.svg)](https://github.com/stevennoyolav4504-dev/Harmony/releases)
...
2.0.1 修复链接输入框占位提示遮挡编辑区的问题：点击占位文字即可聚焦输入框，光标自动落到末尾。

- [2.0.1 下载](https://github.com/stevennoyolav4504-dev/Harmony/releases/tag/v2.0.1)
...
1. **下载**：前往 [Releases](https://github.com/stevennoyolav4504-dev/Harmony/releases) 下载最新版本的 `Harmony-2.0.1-Windows-x64.zip`
2. **解压**：解压到你想要的任意位置
...
如果你想自己运行源码或进行二次开发（项目文件位于仓库的 `Harmony/` 目录）：
...
在 `Harmony/Harmony` 目录（含 `requirements.txt` 与 `src\`）下执行：
...
---
🙏 鸣谢
Harmony 能够顺利诞生，离不开以下优秀开源项目的支撑：

核心依赖
yt-dlp — YouTube 下载的核心引擎，没有它，YouTube 模块无法实现。感谢开发团队持续与 YouTube 的反爬机制“斗智斗勇”。

Playwright — 让 Python 能够驱动真实浏览器，Instagram 提取的精准度依赖于它的能力。

CustomTkinter — 为 Tkinter 赋予了现代化的外观，让 Harmony 的界面拥有了暗色/亮色主题和流畅的视觉体验。

playwright-stealth — 帮助 Harmony 更好地“隐身”于浏览器环境中，降低了被反爬检测的概率。

pysubs2 — 字幕处理的得力助手，让 SRT 的解析与生成变得简单可靠。

基础工具链
Pillow — 图像处理的中流砥柱，缩略图生成、卡片渲染都离不开它。

requests — 最优雅的 HTTP 库，下载、API 请求的基础。

openai — 为未来可能集成的 AI 翻译功能预留了接口（虽然暂时还在路上）。

开源精神
感谢所有为开源社区贡献代码的开发者们。你们的每一行代码，都在让这个世界变得更好。

如果 Harmony 帮到了你，不妨也给这些项目点个 ⭐，让它们被更多人看到。
...
本项目采用 [MIT License](Harmony/LICENSE)。
